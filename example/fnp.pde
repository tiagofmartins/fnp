/*────────────────────────────┬────────────────────────────┐
 │ FeedNPlay (fnp)            │ © Tiago Martins 2025/05/13 │
 ├────────────────────────────┴────────────────────────────┤
 │ The code below enables the automatic execution and      │
 │ positioning of this Processing sketch on FeedNPlay.     │
 ├─────────────────────────────────────────────────────────┤
 │ Please don't change any of the code below.              │
 │ If you have any suggestions or special requests         │
 │ please contact the FeedNPlay team and we will be happy  │
...
final int NOT_SPECIFIED = -99999;

void feednplaySize(int x, int y, int w, int h, String renderer, boolean undecorated) {
  ...
  if (!calledFromSettings) {
    throw new RuntimeException("Function feednplaySize() must be called inside the function settings()");
  }

  // Check if this method is called only once
  if (fnpSizeCalled) {
    throw new RuntimeException("Function feednplaySize() should be called only once");
  }
  fnpSizeCalled = true;
  ...
  if (!fnpArguments.hasKey("x")) {
    if (x != NOT_SPECIFIED && y != NOT_SPECIFIED) {
...
void feednplaySize(int x, int y, int w, int h, String renderer) {
  feednplaySize(x, y, w, h, renderer, false);
}

void feednplaySize(int w, int h, String renderer, boolean undecorated) {
  feednplaySize(NOT_SPECIFIED, NOT_SPECIFIED, w, h, renderer, undecorated);
}

void feednplaySize(int w, int h, String renderer) {
  feednplaySize(w, h, renderer, false);
}

void feednplayFullScreen(String renderer) {
  feednplaySize(0, 0, displayWidth, displayHeight, renderer, true);
}
